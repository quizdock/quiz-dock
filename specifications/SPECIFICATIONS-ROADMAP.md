...
| **v0.2.0** | Builder + Auth | CRUD quiz/questions (REST+OpenAPI+Orval), Keycloak hôte, upload média, UI builder | v0.1.0 | L |
...
| P1-BACK-3 | Abstraction **`AuthProvider`** (`NoAuthProvider`/`KeycloakProvider`) + `AUTH_MODE` (technique §1) |
...
| ID | Tâche |
|----|-------|
| P2-BACK-1 | Intégration **Keycloak** : validation JWT, realm `host`/`player`, import realm (technique §16) |
...
| P2-FRONT-1 | **Connexion** (OIDC Keycloak / mode local) |
